...
export function CastingAssistant() {
  ...
  const suggestionPool = latestResponse?.suggestions?.length ? latestResponse.suggestions : ASSISTANT_SUGGESTIONS
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="relative shrink-0 overflow-hidden border-b border-white/10 bg-[radial-gradient(circle_at_top,_rgba(251,191,36,0.24),_transparent_52%),linear-gradient(135deg,rgba(15,23,42,0.98),rgba(30,41,59,0.94))] px-4 pb-3.5 pt-3.5 sm:px-5">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/10 shadow-inner shadow-white/10 ring-1 ring-white/10">
            ...
            <div ref={scrollRef} className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto bg-[linear-gradient(180deg,rgba(15,23,42,0.98),rgba(2,6,23,0.96))] px-3 py-3 sm:px-4">
              {hasOnlyWelcomeMessage && !loading && (
                <div className="rounded-[24px] border border-white/10 bg-white/[0.04] p-3.5 shadow-[0_12px_35px_rgba(2,6,23,0.22)]">
                  <div className="flex items-start gap-3">
                    <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl border border-amber-300/20 bg-amber-400/10 text-amber-200">
              ...
              {messages.map((message) => {
                ...
                return (
                  <div key={message.id} className={cn('flex', isAssistant ? 'justify-start' : 'justify-end')}>
                    <div
                      className={cn(
                        'max-w-[90%] rounded-[24px] px-3.5 py-3 shadow-[0_14px_35px_rgba(2,6,23,0.18)] transition-colors sm:max-w-[88%]',
                        isAssistant
                          ? 'border border-white/10 bg-white/[0.05] text-slate-100'
                          : 'border border-amber-300/20 bg-gradient-to-br from-amber-500 to-amber-600 text-white',
                      )}
                    >
                      ...
                      <p className="mt-2 text-[13px] leading-[1.6] sm:text-[13.5px]">{message.text}</p>

                      {message.response?.cards?.length ? (
                        <div className="mt-3 space-y-2.5">
                          {message.response.cards.map((card, index) => (
                            <div key={`${card.title}-${index}`} className="rounded-[20px] border border-white/10 bg-slate-950/55 p-3 backdrop-blur-sm">
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  <p className="text-[13px] font-semibold text-white">{card.title}</p>
                                  <p className="mt-1 text-[12px] leading-5 text-slate-300">{card.subtitle}</p>
                                </div>
                                <div className="flex max-w-[45%] flex-wrap justify-end gap-1.5">
                                  {card.chips.map((chip) => (
                                    <span
                                      key={chip}
                                      className="rounded-full border border-amber-300/15 bg-amber-400/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-amber-100"
                                    >
                                      {chip}
                                    </span>
                                  ))}
                              ...
                              {card.meta.length > 0 && (
                                <ul className="mt-2.5 space-y-1 text-[12px] leading-5 text-slate-300">
                                  {card.meta.map((line) => (
                      ...
                      <p className="mt-2 text-[10px] text-inherit/60">{formatRelativeTime(message.createdAt)}</p>
                    </div>
                  </div>
                )
              })}
              ...
              {loading && (
                <div className="flex justify-start">
                  <div className="rounded-[22px] border border-white/10 bg-white/[0.05] px-3.5 py-3 text-[13px] text-slate-300 shadow-[0_10px_30px_rgba(2,6,23,0.18)]">
                    <div className="flex items-center gap-2">
                      <span className="h-2 w-2 animate-pulse rounded-full bg-amber-300" />
                      <span>Reading casting data…</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {voiceState === 'processing' && interimTranscript && (
                <div className="flex justify-end">
                  <div className="max-w-[90%] rounded-[22px] border border-amber-300/20 bg-amber-500/10 px-3.5 py-3 text-amber-100 sm:max-w-[88%]">
                    <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.2em] text-amber-200/70">
                      <Mic className="h-3 w-3" />
                      <span>Voice</span>
                    </div>
                    <p className="mt-2 text-[13px] leading-[1.55] italic opacity-85">{interimTranscript}</p>
                    <p className="mt-1.5 text-[10px] text-amber-200/50">Sending…</p>
                  </div>
                </div>
              )}
              ...
              {voiceState === 'error' && (
                <div className="flex justify-center">
                  <div className="rounded-[20px] border border-red-300/20 bg-red-500/10 px-3 py-2.5 text-[13px] text-red-200">
            ...
            <div className="shrink-0 border-t border-white/10 bg-slate-950/95 px-3 pb-3 pt-2.5 sm:px-4 sm:pb-4">
              <div className="mb-2.5 rounded-[22px] border border-white/10 bg-white/[0.04] px-3 py-3 shadow-inner shadow-black/10">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2.5 py-1.5 text-[12px] font-medium text-slate-300 transition hover:border-amber-300/25 hover:text-white"
                  >
                      ...
                      <div className="flex flex-wrap gap-2 pb-1 pt-3">
                        {suggestionPool.map((suggestion) => (
                          <button
                            ...
                            className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-left text-[11px] font-medium leading-4 text-slate-200 transition hover:border-amber-300/40 hover:bg-amber-400/10 hover:text-white"
                          >
                            {suggestion}
                          </button>
                        ))}
              ...
              {isListening && liveTranscript && (
                <motion.div
                  ...
                  className="mb-2.5 rounded-[18px] border border-amber-300/20 bg-amber-400/10 px-3 py-2"
                >
                  <p className="text-[12px] leading-relaxed text-amber-200">{liveTranscript}</p>
                </motion.div>
              )}

              <div className="rounded-[26px] border border-white/10 bg-white/[0.05] p-1.5 shadow-inner shadow-black/10">
                <div className="flex items-end gap-1.5">
                  <button
                    ...
                    className={cn(
                      ...
                      isListening
                        ? 'animate-pulse border border-red-300/30 bg-red-500/20 text-red-400'
                        : voiceState === 'unsupported'
                  ...
                  <label className="sr-only" htmlFor="casting-assistant-input">
                    Ask the assistant
                  </label>
                  <textarea
                    ...
                    className={cn(
                      'min-h-[40px] flex-1 resize-none bg-transparent px-2 py-2 text-[13px] text-white placeholder:text-slate-400 focus:outline-none',
