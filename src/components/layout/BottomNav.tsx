...
export function BottomNav() {
  ...
  const activePrimaryLabel = mainItems.find((item) => item.path && item.path !== '#' && (location.pathname === item.path || (item.path !== '/dashboard' && location.pathname.startsWith(item.path))))?.label || 'More'
          ...
          {mainItems.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex h-16 w-16 flex-col items-center justify-center gap-1 transition-colors focus:outline-none',
                  isActive ? 'text-amber-600' : 'text-slate-500 active:text-slate-700'
                )}
              >
                <item.icon className="h-5 w-5" />
                <span className="text-[10px] font-medium">{item.label}</span>
                {item.hint && <span className="text-[9px] text-slate-400">{item.hint}</span>}
              </button>
            )
          })}
        </div>
              ...
              <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50/80 px-4 py-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-slate-600">You are currently navigating from {activePrimaryLabel}. Use this sheet for records, reporting, and admin surfaces.</p>
                  </div>
